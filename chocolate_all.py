...
def prepareData(df, ids_fn, one_hot=False, rfa=False):
...
def load_or_gen_data(datafn, names, ids_fn, ohe=False, rfa=False):
    folder, _ = os.path.split(datafn)
    check = os.path.join(folder, '_'.join(name.rsplit('.')[0] for name in names) + '.pickle')
    if os.path.isfile(check):
        ...
        dfs = []
        for name in names:
            dfs.append(readZipData(datafn, name, dropNan=True))
        d = prepareData(pd.concat(dfs), ids_fn, ohe, rfa=rfa)
...
if __name__ == "__main__":
    ...
    dstring = '_'.join(name.rsplit('.')[0] for name in datafns)
    for i, lh in enumerate(labelHour):
        trn_x, trn_y, tst_x, tst_y = load_or_gen_data(datapath, datafns, ids_fn, rfa=True)
        # print(np.sum(np.isnan(trn_x)))
        data.append((trn_x, trn_y[:,i], tst_x, tst_y[:,i], 'Data_%sLabel_%d' % (dstring, lh)))
