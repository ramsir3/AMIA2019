import os, datetime, sys, json, gc
...
def runTraining(cfgfn):
    ...
    #     print(sess.run())
        ...
        while (lss_change is None) or (lss_change > loss_thresh and step < max_train_steps):
            ...
            for batch in trainBatches.setSeed(seed):
                ...
                if headers is None:
                    ...
                    # print(headers)
                    _, trn_X, trn_Y, hdrscut = prepareData(batch, headers, nclasses=4, labelCol=labelCol, labelStart=labelStart, debug=True) # 
                    print(hdrscut)
                else:
                    # batch, _, row_nan_bool = dropNanCols(batch, None, row_nan_bool)
                    _, trn_X, trn_Y, hdrscut = prepareData(batch, headers, nclasses=4, labelCol=labelCol, labelStart=labelStart)
            ...
            print('X:', sys.getsizeof(trn_X), 'Y:', sys.getsizeof(trn_Y))
            gc.collect()
        ...
        print("Step %5d | L2 Loss = %.4f, Train Accuracy = %.3f" % (step, trn_lss, trn_acc))
        
        # testing
        for test in testBatches.setSeed(seed):  
            # test, _, _ = dropNanCols(batch, None, row_nan_bool)
            _, tst_X, tst_Y, _ = prepareData(test, headers, nclasses=4, labelCol=labelCol, labelStart=labelStart)
...
if __name__ == "__main__":
    cfgfn = None if len(sys.argv) <= 1 else sys.argv[1]
    runTraining(cfgfn)